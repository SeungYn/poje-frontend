...
export default function JobPortfolioPagingFooter(data: PropsType) {
	...
	return (
		<PageList>
			...
			{pageArr.map(i => <PageItem isCurrentPage={String(i) ===  currentPage  } onClick={() => navigate(`/job/${type}/${i}`)}>{i}</PageItem>)}
...
const PageItem = styled.li<{isCurrentPage:boolean}>`
	...
	border:${({isCurrentPage})=> isCurrentPage ? '1px solid black' : 'none'}
