...
import { NoteListType, NoteType } from '@src/service/types/member';
import NoteItem from './NoteItem';
import { useState } from 'react';
import { AiOutlineSend } from 'react-icons/ai';
import { useGetNote } from '@src/hooks/note';

type propsType = {
  selectedNote: NoteListType;
};

export default function NoteContent({ selectedNote }: propsType) {
  const { handleDeleteSelectedNote, handleReplyNote } = useNoteContext();
  const [inputMessage, setInputMessage] = useState('');
  const noteList = useGetNote(selectedNote!.portfolioId!);
  const data: NoteType[] = [
    {
      id: 34,
      message: '누구세요???',
