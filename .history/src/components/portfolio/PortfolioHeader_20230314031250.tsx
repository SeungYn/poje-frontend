...
const Title = styled.h1<{ isIntroDisapper: boolean; isOpen: boolean }>`
  ...
  &:hover {
    color: ${({ isIntroDisapper, isOpen }) =>
      isIntroDisapper || isOpen ? '#7c7c7c' : 'white'};
  }
  cursor: pointer;
`;
...
const HamburgerButton = styled.button<{
  isIntroDisapper: boolean;
  isOpen: boolean;
}>`
  position: absolute;
  display: none;
  right: 1rem;
  color: ${({ theme, isOpen, isIntroDisapper }) =>
    isIntroDisapper || isOpen ? 'black' : theme.textColorToneDown};
  font-size: 3rem;
  top: 1rem;
  &:hover {
    color: ${({ isIntroDisapper, isOpen }) =>
