...
export type IconsKeyType = keyof IconsType;
...
function extractPathJobType(path: string): IconsKeyType | null {
  // 직업식별자에 포함된 이미지의 식별자를 가져오기
  let result = path.replace('./', '');
  const splitPathList = result.split('/');
  if (splitPathList.length === 1) return null;
  return splitPathList[0] as IconsKeyType;
}
export const iconKeys: IconsKeyType[] = Object.keys(icons) as IconsKeyType[];
