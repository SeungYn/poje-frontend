...
export const useModifyMyInfo = () => {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const imgUrlExcludeData = Object.fromEntries(
      Object.entries(form).filter(([key, value]) => key !== 'profileImg')
    );
    console.log(imgUrlExcludeData);
  };
...
export const useUpdateMemberInfo = () => {
  const updateMemberInfo = useMutation((data: PutMemberInfoRequest) => {
